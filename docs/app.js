...
function initCharts() {
  ...
  function tickDots(){var now=Date.now();for(var k in pageTimes){var age=(now-pageTimes[k])/1000;var dot=document.getElementById('d-'+k);if(!dot)continue;var c;if(age<2)c=lerpColor('#00ff00','#ffff00',(age)/2);else if(age<15)c=lerpColor('#ffff00','#ff4400',(age-2)/13);else if(age<60)c=lerpColor('#ff4400','#ff0000',(age-15)/45);else c='#444444';dot.style.background=c;dot.style.opacity=Math.max(0.3,Math.exp(-age/30))}var rd=document.getElementById('dot-rkt'),rv=document.getElementById('val-rkt'),rds=document.getElementById('dot-rkt-sticky'),rvs=document.getElementById('val-rkt-sticky');if(!livePktCount){rd.className='dot dd';rv.textContent='no pkts';if(rds)rds.className='dot dd';if(rvs)rvs.textContent='no pkts'}else{var a=(Date.now()-lastPktMs)/1000;if(a<3){rd.className='dot dg';rv.textContent=a.toFixed(1)+'s';if(rds)rds.className='dot dg';if(rvs)rvs.textContent=a.toFixed(1)+'s'}else if(a<15){rd.className='dot dy';rv.textContent=a.toFixed(0)+'s';if(rds)rds.className='dot dy';if(rvs)rvs.textContent=a.toFixed(0)+'s'}else{rd.className='dot dr';rv.textContent=a.toFixed(0)+'s';if(rds)rds.className='dot dr';if(rvs)rvs.textContent=a.toFixed(0)+'s'}}}
  ...
  async function bleReadStatus() {
    ...
    try {
      ...
      if (typeof s.baseBattMv === 'number') {
        ...
        var bes = document.getElementById('val-basebatt-sticky');
        if (bes) { bes.textContent = s.baseBattMv + 'mV'; bes.style.color = be.style.color; }
      }
  ...
  async function rktReadStatus() {
    ...
    try {
      ...
      if (typeof s.batt === 'number') {
        ...
        var bes = document.getElementById('val-rktbatt-sticky');
        if (bes) { bes.textContent = s.batt + 'mV'; bes.style.color = be.style.color; }
      }
